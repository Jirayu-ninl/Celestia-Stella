...
import { advLogger } from '../../advLogger'
...
enum InternalErrorCodes {
  ...
  Server_2 = 'Server_2', // Error handler received an error of type Error and not AppError
}
...
const normalizeError = (errorToHandle: unknown): AppError => {
  ...
  return new AppError(
    'general-error',
    `Error Handler received a none error instance with type - ${inputType}, value - ${inspect(
      errorToHandle,
    )}`,
    AppErrorType.Unknown,
...
const terminateHttpServerAndExit = () => {
  // Note: Hono apps don't have a direct .close() method like Node.js HTTP servers
  // You may need to implement server shutdown logic specific to your Hono runtime
  // (e.g., if using Node.js adapter, Bun, Cloudflare Workers, etc.)
  process.exit()
}
...
const metricsExporter = {
  fireMetric: (name: string, labels: object) => {
    advLogger.info('In real production code I will really fire metrics', {
      name,
      labels,
    })
  },
...
const errorHandler = {
  ...
  handleError(errorToHandle: unknown): AppError {
    try {
      ...
      advLogger.error(appError.message, appError)
